...
export class ParentDashboardScene extends Phaser.Scene {
  ...
  showPinEntry() {
    // Title
    this.add.text(200, 80, 'Parent Dashboard', {
      fontSize: '24px',
      fill: '#f7dc6f',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    this.add.text(200, 120, 'Enter PIN to access', {
      fontSize: '14px',
      fill: '#81ecec',
      fontFamily: 'Arial'
    }).setOrigin(0.5);
    ...
    this.pinDisplay = this.add.text(200, 180, '_ _ _ _', {
      fontSize: '36px',
      fill: '#ffffff',
    ...
    // Back button
    const backBtn = this.add.text(200, 620, '< Back to Game', {
      fontSize: '16px',
      fill: '#888888',
      fontFamily: 'Arial'
    ...
    // Hint text
    this.add.text(200, 660, 'Default PIN: 0000', {
      fontSize: '12px',
      fill: '#555555',
      fontFamily: 'Arial'
    }).setOrigin(0.5);
  ...
  showDashboard() {
    ...
    // Header
    this.add.rectangle(200, 40, 400, 80, 0x2d2d44);

    this.add.text(200, 25, 'Parent Dashboard', {
      fontSize: '20px',
      fill: '#f7dc6f',
      fontFamily: 'Arial',
      fontStyle: 'bold'
  ...
  createTabs() {
    ...
    tabs.forEach((tab, index) => {
      ...
      const bg = this.add.rectangle(x, 60, tabWidth - 10, 30, isActive ? 0x4ecdc4 : 0x2d2d44)
        .setStrokeStyle(1, 0x4ecdc4)
        .setInteractive();

      const text = this.add.text(x, 60, tab.label, {
        fontSize: '12px',
  ...
  addStatCard(y, label, value, accentColor) {
    const card = this.add.rectangle(200, y, 360, 55, 0x2d2d44, 0.9)
      .setStrokeStyle(2, accentColor);
    this.contentContainer.add(card);

    const labelText = this.add.text(40, y - 8, label, {
      fontSize: '14px',
      fill: '#888888',
      fontFamily: 'Arial'
    });
    ...
    const valueText = this.add.text(360, y, value, {
      fontSize: '24px',
      fill: '#ffffff',
      fontFamily: 'Arial',
      fontStyle: 'bold'
  ...
  showAnalyticsTab() {
    ...
    // Title: Fact Mastery
    const title = this.add.text(200, y, 'Multiplication Fact Mastery', {
      fontSize: '16px',
      fill: '#f7dc6f',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    // Most Missed Facts
    const missedTitle = this.add.text(200, y, 'Most Missed Facts', {
      fontSize: '16px',
      fill: '#ff6b6b',
      fontFamily: 'Arial',
      fontStyle: 'bold'
  ...
  createMasteryHeatMap(startY) {
    ...
    for (let row = 1; row <= 10; row++) {
      const rowY = startY + row * (cellSize - 6);

      // Row header
      const rowHeader = this.add.text(startX - 5, rowY + cellSize / 2, row.toString(), {
        fontSize: '12px',
        fill: '#888888',
        fontFamily: 'Arial'
      }).setOrigin(1, 0.5);
      ...
      for (let col = 1; col <= 10; col++) {
        ...
        const cell = this.add.rectangle(cellX, cellY, cellSize - 4, cellSize - 8, color);
    ...
    // Legend
    const legendY = startY + 11 * (cellSize - 6) + 10;
    const legendColors = [
    ...
    legendColors.forEach(({ color, label }) => {
      const box = this.add.rectangle(legendX, legendY, 15, 15, color);
      this.contentContainer.add(box);

      const text = this.add.text(legendX + 12, legendY, label, {
        fontSize: '10px',
        fill: '#888888',
        fontFamily: 'Arial'
      }).setOrigin(0, 0.5);
  ...
  showMostMissedFacts(y) {
    ...
    if (missedFacts.length === 0) {
      const noData = this.add.text(200, y + 20, 'No data yet - keep playing!', {
        fontSize: '14px',
        fill: '#555555',
        fontFamily: 'Arial'
      }).setOrigin(0.5);
    ...
    missedFacts.forEach((fact, index) => {
      const text = this.add.text(200, y + index * 25, `${fact.a} × ${fact.b} = ${fact.a * fact.b}  (${fact.accuracy}% accuracy)`, {
        fontSize: '14px',
  ...
  showSettingsTab() {
    ...
    const diffTitle = this.add.text(200, y, 'Difficulty Adjustment', {
      fontSize: '16px',
      fill: '#f7dc6f',
      fontFamily: 'Arial',
      fontStyle: 'bold'
  ...
  addSettingButton(y, label, callback, color = 0x4ecdc4) {
    const btn = this.add.rectangle(200, y, 300, 45, 0x2d2d44)
  ...
  showChangePinDialog() {
    ...
    const panel = this.add.rectangle(200, 300, 300, 250, 0x2d2d44)
      .setStrokeStyle(2, 0x4ecdc4);

    const title = this.add.text(200, 200, 'Change PIN', {
      fontSize: '18px',
      fill: '#f7dc6f',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    const instruction = this.add.text(200, 240, 'Enter new 4-digit PIN:', {
      fontSize: '14px',
      fill: '#ffffff',
      fontFamily: 'Arial'
    }).setOrigin(0.5);
    ...
    this.newPinDigits = ['', '', '', ''];
    this.newPinIndex = 0;
    const pinDisplay = this.add.text(200, 280, '_ _ _ _', {
      fontSize: '24px',
      fill: '#ffffff',
      fontFamily: 'monospace'
    }).setOrigin(0.5);
    ...
    const numRow = this.add.text(200, 330, '1 2 3 4 5 6 7 8 9 0', {
      fontSize: '20px',
      fill: '#4ecdc4',
      fontFamily: 'monospace'
    }).setOrigin(0.5).setInteractive();
    ...
    // Cancel button
    const cancelBtn = this.add.text(130, 390, 'Cancel', {
      fontSize: '16px',
      fill: '#ff6b6b',
      fontFamily: 'Arial'
    }).setOrigin(0.5).setInteractive();
    ...
    // Save button
    const saveBtn = this.add.text(270, 390, 'Save', {
      fontSize: '16px',
      fill: '#58d68d',
      fontFamily: 'Arial'
    }).setOrigin(0.5).setInteractive();
    ...
    saveBtn.on('pointerdown', () => {
      ...
      if (this.newPinIndex === 4) {
        ...
        // Show confirmation
        const confirm = this.add.text(200, 350, 'PIN Updated!', {
          fontSize: '18px',
          fill: '#58d68d',
          fontFamily: 'Arial',
          fontStyle: 'bold'
  ...
  showResetConfirmation() {
    ...
    const panel = this.add.rectangle(200, 300, 320, 200, 0x2d2d44)
      .setStrokeStyle(2, 0xff6b6b);

    const title = this.add.text(200, 230, 'Reset All Progress?', {
      fontSize: '18px',
      fill: '#ff6b6b',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    const warning = this.add.text(200, 280, 'This will delete ALL game progress,\nincluding levels, stars, achievements,\nand learning data.\n\nThis cannot be undone!', {
      fontSize: '12px',
      fill: '#ffffff',
      fontFamily: 'Arial',
      align: 'center',
    ...
    // Cancel button
    const cancelBtn = this.add.text(130, 370, 'Cancel', {
      fontSize: '16px',
      fill: '#4ecdc4',
      fontFamily: 'Arial'
    }).setOrigin(0.5).setInteractive();
    ...
    // Confirm button
    const confirmBtn = this.add.text(270, 370, 'Reset', {
      fontSize: '16px',
      fill: '#ff6b6b',
      fontFamily: 'Arial',
      fontStyle: 'bold'
    ...
    confirmBtn.on('pointerdown', () => {
      ...
      const confirm = this.add.text(200, 350, 'Progress Reset!', {
        fontSize: '18px',
        fill: '#ff6b6b',
        fontFamily: 'Arial',
        fontStyle: 'bold'
