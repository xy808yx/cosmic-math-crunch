// Per-world background palette + horizon silhouette for the play scenes.
// Each world gets a distinct vertical gradient plus a hand-rolled horizon shape
// so worlds read as different places without any new sprite assets and without
// tinting the HUD chrome.
//
// Read by GameScene (the play screen), ConveyorScene (the Chapter 3 belt, which
// lifts the gradient toward daylight and draws the horizon as its back wall) and
// LevelSelectScene (the sky behind the mission cards). The world map has its own
...
// Chapter 3 "Home Ground": worlds 31 to 38 are eight real places on one family
// Saturday, morning to dusk, drawn in the Paper Cutout style. Their art lives in
// src/homeGround (one module per world plus shared helpers in paper.js) and is
// registered here by id so getWorldBackground keeps its signature. Each horizon
// is anchored at the line (opts.y), draws upward with only a shallow floor
// below, and never paints an opaque sheet over the sky the scene already made.
// Plain hand-built shapes only, no spirals, sunbursts or rays (content rule).
for (const id of Object.keys(HOME_GROUND_WORLDS)) {
  const world = HOME_GROUND_WORLDS[id];
  WORLD_BACKGROUNDS[id] = {
    bgTop: world.bgTop,
    bgBottom: world.bgBottom,
    drawHorizon: (scene, opts) => world.drawHorizon(scene, opts)
  };
}
